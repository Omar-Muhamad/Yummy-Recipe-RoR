...
RSpec.describe 'The recipes index page', type: :feature do
  describe 'after logging in' do
    before :each do
      @user1 = User.create!(name: 'Test User', email: 'test@testmail.com', password: '123456', confirmed_at: Time.now)

      @recipe1 = Recipe.create!(name: 'Test Recipe 1', description: 'test description 1', public: true, preparation_time: 10, cooking_time: 10, user_id: @user1.id)

      @recipe2 = Recipe.create!(name: 'Test Recipe 2', description: 'test description 2', public: false, user_id: @user1.id)
  ...
  it 'shows the title, description, preparation and cooking time' do
    ...
    expect(page).to have_content 'Preparation: test description 2'
  end
